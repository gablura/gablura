...
export default function PackageExamples({ pkg }: PackageExamplesProps) {
  const docs = pkg.documentation;
  const examples = docs?.examples || "";

  // Parse examples into separate code blocks
  // Examples can be separated by double newlines or markdown-style code blocks
  const parseExamples = (raw: string) => {
    ...
    while ((match = codeBlockRegex.exec(raw)) !== null) {
      // Add any text before this code block as a separate example
    ...
    // If no code blocks found, split by double newlines
    ...
    <div className="rounded-xl border border-border-subtle bg-surface p-6 sm:p-8">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Learn how to use {pkg.name} with these examples.
        </p>
      </div>
