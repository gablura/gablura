...
function loadKey(envKey: string | undefined, filePath: string): string {
  if (envKey) return envKey;
  return readFileSync(join(process.cwd(), filePath), "utf-8");
}

const privateKey = loadKey(process.env.AUTH_PRIVATE_KEY, "keys/private.pem");
const publicKey = loadKey(process.env.AUTH_PUBLIC_KEY, "keys/public.pem");
